...
async def test_get_info(helpers, fb_api):
    ...
    with respx.mock:
        respx.get(f"https://graph.facebook.com/{fb_api.version}/{cm_id}").mock(
...
async def test_get_batch(helpers, fb_api):
    ...
    with respx.mock:
        ...
        comments_json = await fb_api.comment.get_batch(
            ids=cm_ids,
            fields="id,comment_count,created_time,like_count,message,permalink_url",
            return_json=True,
        )
...
@pytest.mark.asyncio
async def test_create_comment(helpers, fb_api):
    object_id = "post_id"
    with respx.mock:
        respx.post(f"https://graph.facebook.com/{fb_api.version}/{object_id}/comments").mock(
            return_value=respx.MockResponse(
                status_code=200,
                json=helpers.load_json(
        ...
        comment = await fb_api.comment.create(
            object_id=object_id,
        ...
        comment_json = await fb_api.comment.create(
            object_id=object_id,
...
@pytest.mark.asyncio
async def test_update_comment(helpers, fb_api):
    comment_id = "245778718206154_229887283291835"
    with respx.mock:
        respx.post(f"https://graph.facebook.com/{fb_api.version}/{comment_id}").mock(
            return_value=respx.MockResponse(
                status_code=200,
                json=helpers.load_json(
                    "testdata/facebook/apidata/comments/create_resp.json"
    ...
    with respx.mock:
        ...
        comment_hidden = await fb_api.comment.update(
            comment_id=comment_id,
            is_hidden=True,
        )
...
@pytest.mark.asyncio
async def test_delete_comment(fb_api):
    comment_id = "245778718206154_229887283291835"
    with respx.mock:
        respx.delete(f"https://graph.facebook.com/{fb_api.version}/{comment_id}").mock(
            return_value=respx.MockResponse(
                status_code=200,
                json={"success": True},
            ),
